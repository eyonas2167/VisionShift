...
public:
  ...
  virtual bool build()=0; //initialize the planner (abstrat method)

  //finding a path (abstrat method)
  virtual bool find_path( const Point2d& start, const Point2d& goal, std::list<Point2d>& path )=0;

  //TODO: shorten and smooth the path (20 pts)
  //To smooth, use Quadratic Bezier curves
  void shorten_and_smooth(std::list<Point2d>& path);
